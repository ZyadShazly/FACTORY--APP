...
edit('src/assets/AssetsPage.jsx', [
  ...
  { name:'assignment resend action double quotes', search:'{profile.role==="owner"&&<AssignmentEmergencyActions assignment={a} quantity={quantity} onSelect={setEmergency}/>}','replace':'{a.status==="pending_receiver_confirmation"&&permissions.assets_issue&&<Button variant="ghost" onClick={()=>renewLink(a.id,"issue",a)}><ExternalLink size={14}/> إرسال/إعادة إرسال الرابط</Button>}{profile.role==="owner"&&<AssignmentEmergencyActions assignment={a} quantity={quantity} onSelect={setEmergency}/>}' },
