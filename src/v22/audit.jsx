...
export function AuditLogTab({ data }) {
  ...
  const rows=useMemo(()=>data.auditLog.filter((r)=>(!table||r.table_name===table)&&(!action||r.action===action)&&(!search||`${r.record_id} ${JSON.stringify(r.new_data||{})}`.toLowerCase().includes(search.toLowerCase()))).sort((a,b)=>new Date(b.created_at)-new Date(a.created_at)),[data.auditLog,table,action,search]);
  return <div><PageTitle eyebrow="الحوكمة والأمان" title="سجل التدقيق" description="سجل غير قابل للتعديل لكل الإضافات والتغييرات والحذف وعمليات الدفع."/><Panel><div className="v22-filters"><Input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="بحث بالمعرف أو البيانات..."/><Select value={table} onChange={(e)=>setTable(e.target.value)}><option value="">كل الوحدات</option>{Object.entries(TABLE_LABELS).map(([k,v])=><option key={k} value={k}>{v}</option>)}</Select><Select value={action} onChange={(e)=>setAction(e.target.value)}><option value="">كل الإجراءات</option>{Object.entries(ACTION_LABELS).map(([k,v])=><option key={k} value={k}>{v}</option>)}</Select></div>{rows.length?<DataTable headers={["الوقت","الوحدة","الإجراء","السجل","المستخدم","ملخص التغيير"]}>{rows.map((r)=><tr key={r.id}><td>{new Date(r.created_at).toLocaleString("ar-EG")}</td><td>{TABLE_LABELS[r.table_name]||r.table_name}</td><td><span className={`audit-action ${r.action}`}>{ACTION_LABELS[r.action]||r.action}</span></td><td><code>{r.record_id?.slice(0,12)}</code></td><td><code>{r.actor_id?.slice(0,8)||"النظام"}</code></td><td className="audit-summary">{summarize(r)}</td></tr>)}</DataTable>:<EmptyState title="لا توجد أحداث مطابقة"/>}</Panel></div>;
}
