...
export function ProjectWorkspace({ project, data, profile, permissions, refresh, onBack, FileUploader, ProjectTimeline }) {
  ...
  return <div className="project-workspace" dir="rtl">
    ...
    <nav className="workspace-tabs" aria-label="أقسام مساحة المشروع">{TABS.map(([id,label,Icon])=><button key={id} className={tab===id?"active":""} onClick={()=>setTab(id)}><Icon size={15}/><span>{label}</span>{["budget","reports"].includes(id)&&<small>قريبًا</small>}</button>)}</nav>
