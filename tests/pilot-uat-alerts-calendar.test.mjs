...
test("asset alerts are loaded through a permission-aware RPC", async () => {
  ...
  const dataBootstrap = await readFile(dataBootstrapUrl, "utf8");
  ...
  assert.match(dataBootstrap, /key === "assetAlerts"[\s\S]*get_asset_alerts_visible/);
  assert.match(app, /createTableFetcher, EMPTY_DATA/);
  assert.doesNotMatch(app, /key === "assetAlerts"[\s\S]*get_asset_alerts_visible/);
});
