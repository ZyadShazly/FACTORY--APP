-- Validate partial procurement receipts under row locks before inventory posting.
begin;
...
begin
  if actor is null or public.current_identity_role() not in ('owner','manager','accountant','production') or not public.is_current_profile_active() then
  ...
  values(po_id,actor,'confirmed',nullif(btrim(payload->>'supplier_delivery_reference'),''),nullif(btrim(payload->>'notes'),''),actor,statement_timestamp())
  returning * into gr;
  ...
  set status=case when not exists(
    select 1 from public.purchase_order_items where purchase_order_id=po_id and received_quantity<quantity
...
revoke all on function public.confirm_goods_receipt(jsonb) from public,anon,authenticated;
grant execute on function public.confirm_goods_receipt(jsonb) to authenticated;
