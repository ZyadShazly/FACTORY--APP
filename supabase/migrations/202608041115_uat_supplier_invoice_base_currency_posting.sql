-- UAT-006: preserve document currency and post approved supplier invoice costs in base currency.
-- Additive replacement of the protected approval RPC; no historical rows are rewritten.
begin;
...
begin
  ...
  where id=(payload->>'purchase_order_id')::uuid
  for update;

  if not found or po.status not in('fully_received','partially_received') then
  ...
  if po.currency<>effective_base_currency and effective_rate_date is null then
    raise exception 'Exchange-rate date is required for foreign-currency invoice approval';
  end if;
  if po.currency=effective_base_currency and effective_rate<>1 then
    raise exception 'Exchange rate must equal 1 when document and base currencies match';
  end if;

  insert into public.supplier_invoices(
    invoice_number,supplier_id,purchase_order_id,project_id,invoice_date,due_date,
    currency,base_currency,exchange_rate,rate_date,status,notes,created_by
  ) values(
    payload->>'invoice_number',po.supplier_id,po.id,po.project_id,
    (payload->>'invoice_date')::date,nullif(payload->>'due_date','')::date,
    po.currency,effective_base_currency,effective_rate,effective_rate_date,
    'submitted',payload->>'notes',actor
  ) returning * into inv;
  ...
  for item in select * from jsonb_array_elements(coalesce(payload->'items','[]'::jsonb)) loop
    ...
    where gr.purchase_order_id=po.id
      ...
      and gri.purchase_order_item_id=po_item.id;
    ...
    insert into public.supplier_invoice_lines(
      supplier_invoice_id,purchase_order_item_id,goods_receipt_item_id,description,
      quantity,unit_price,discount_amount,tax_amount,budget_item_id,milestone_id,cost_center_reference
    ) values(
    ...
    entry:=public.save_project_actual_cost(jsonb_build_object(
      ...
      'cost_category','purchase_invoice',
      ...
      'metadata',jsonb_build_object(
        ...
        'document_amount',line_total_value,
        'document_currency',po.currency,
        'base_amount',base_line_total,
        'base_currency',effective_base_currency,
    ...
    update public.supplier_invoice_lines
      set actual_cost_entry_id=(entry->>'id')::uuid
      where id=line_id;
  end loop;
...
comment on function public.approve_supplier_invoice(jsonb) is
  'Approves a received supplier invoice, preserves document currency, and posts project actual cost in stored base currency.';
