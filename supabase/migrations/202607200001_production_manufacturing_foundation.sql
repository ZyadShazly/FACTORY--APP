...
do $$ begin
  alter table public.production_orders
    add constraint production_orders_plan_dates_check
...
create table if not exists public.production_material_requirements (
  ...
  budget_item_id uuid references public.project_estimated_budget_items(id) on delete restrict,
...
create trigger production_order_touch_updated_at
before update on public.production_orders
for each row execute function private.touch_production_order();

create or replace function public.plan_production_order(
  target_order uuid,
  start_date date,
...
begin
  if actor is null or role_name not in ('owner','manager') then
    raise exception 'Owner or manager role required';
  end if;
  if start_date is null or (end_date is not null and end_date < start_date) then
    raise exception 'Valid production plan dates required';
  end if;
  update public.production_orders
  set status='planned', planned_start_date=start_date, planned_end_date=end_date,
      cancellation_reason=null, cancelled_at=null, cancelled_by=null
  where id=target_order and status in ('draft','planned')
...
begin
  ...
  if not exists(select 1 from public.production_material_requirements where production_order_id=target_order) then
    raise exception 'At least one material requirement is required';
  end if;
  update public.production_orders
...
begin
  ...
  if not found or req.order_status not in ('released','in_progress') then
    raise exception 'Released production order required';
  end if;
  if req.project_id is null then raise exception 'Production order must be linked to a project'; end if;
  if req.inventory_movement_id is not null or req.issued_quantity<>0 then
    raise exception 'Material requirement already issued';
  end if;
  if issue_quantity > req.required_quantity then raise exception 'Issue exceeds required quantity'; end if;
  ...
  set issued_quantity=issue_quantity,
      consumed_quantity=issue_quantity,
      inventory_movement_id=(movement->>'id')::uuid
...
begin
  ...
  if exists(select 1 from public.production_material_requirements where production_order_id=target_order and issued_quantity<required_quantity) then
    raise exception 'All required materials must be issued before completion';
  end if;
  if exists(select 1 from public.production_order_operations where production_order_id=target_order and status not in ('completed','skipped')) then
    raise exception 'All production operations must be completed or skipped';
  end if;
  update public.production_orders
...
begin
  ...
  for req in
    select inventory_movement_id from public.production_material_requirements
    where production_order_id=target_order and inventory_movement_id is not null
  loop
    if not exists(select 1 from public.inventory_movements where reversed_movement_id=req.inventory_movement_id) then
      perform public.reverse_inventory_movement(req.inventory_movement_id,reason);
    end if;
  end loop;
