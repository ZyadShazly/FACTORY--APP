...
begin
 if auth.uid() is null or not public.is_current_profile_active() or role_name not in ('owner','manager','accountant') then raise exception using errcode='42501',message='Procurement access required'; end if;
 ...
 return jsonb_build_object(
  ...
  'capabilities',jsonb_build_object('request',true,'approve_request',role_name in('owner','manager'),'quote',role_name in('owner','manager','accountant'),'order',role_name in('owner','manager'),'approve_order',role_name in('owner','manager'),'send_order',role_name in('owner','manager','accountant'),'receive',true,'invoice',role_name in('owner','manager'))
 );
end
$$;