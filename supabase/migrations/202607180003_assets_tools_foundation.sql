...
create table public.asset_assignments(
 id uuid primary key default gen_random_uuid(), assignment_code text not null unique default ('CST-'||to_char(clock_timestamp(),'YYMMDD')||'-'||upper(substr(encode(gen_random_bytes(5),'hex'),1,8))),
...
declare a public.assets%rowtype; qty numeric:=coalesce((payload->>'quantity')::numeric,1); qr text:=coalesce(nullif(payload->>'qr_value',''),'NXT-A-'||upper(encode(gen_random_bytes(8),'hex')));
begin
...
declare ass public.asset_assignments%rowtype; emp public.employees%rowtype; item jsonb; a public.assets%rowtype; qty numeric; secret text:=encode(gen_random_bytes(24),'hex'); hours int;
begin
 ...
 values('pending_receiver_confirmation',emp.id,nullif(payload->>'receiver_profile_id','')::uuid,emp.full_name,emp.phone,auth.uid(),nullif(payload->>'project_id','')::uuid,nullif(payload->>'department_id','')::uuid,nullif(payload->>'issue_location_id','')::uuid,btrim(payload->>'purpose'),nullif(payload->>'expected_return_at','')::timestamptz,now(),encode(digest(secret,'sha256'),'hex'),now()+make_interval(hours=>hours),payload->>'notes',auth.uid(),auth.uid()) returning * into ass;
...
begin
 ...
 if a.confirmation_token_hash<>encode(digest(secret,'sha256'),'hex') then
...
declare ev public.asset_return_events%rowtype; i jsonb; ai public.asset_assignment_items%rowtype; qty numeric; pending numeric; secret text:=encode(gen_random_bytes(24),'hex'); hours int; ass public.asset_assignments%rowtype;
begin
 ...
 insert into public.asset_return_events(assignment_id,notes,confirmation_token_hash,confirmation_expires_at) values(ass.id,payload->>'notes',encode(digest(secret,'sha256'),'hex'),now()+make_interval(hours=>hours)) returning * into ev;
...
begin
 ...
 if ev.confirmation_token_hash<>encode(digest(secret,'sha256'),'hex') then update public.asset_return_events set confirmation_failed_attempts=confirmation_failed_attempts+1,confirmation_locked_until=case when confirmation_failed_attempts+1>=5 then now()+interval '15 minutes' end where id=ev.id; return jsonb_build_object('status','invalid'); end if;
